function [root,mu,V,flag_eigs,eig_time,New_res_vec,New_abs_vec,New_rel_vec,Gmres_rel_vec,Gmres_flag_vec] ...
                        = New_Kry_solver_fix_point(X0,Ra,imax,abs_tol,rel_tol)
...
%     gamma = rotation_symmetry(unew,omega,tspan,Nc,K);
    b = -(phi-unew) ; 
          %u_theta_0.'*(unew-u0)];
    Ax = @(v) matvec_prod_parametization_fixed_point(unew,phi,Ra,v);
...
tic
[V,D,flag_eigs] = eigs(@(v) matvec_prod_fixed_point_eig(unew,phi,Ra,v),4*(Nc+1)*(2*K+1),10);    
mu = diag(D);
