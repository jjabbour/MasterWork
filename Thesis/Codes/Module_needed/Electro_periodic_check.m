function [U_perturbation,U_full] = Electro_periodic_check(tspan,U0,Ra,dt,Pr,Re,rr,K,Nc)
...
U_perturbation = zeros(4*(2*K+1)*(Nc+1),Nrun);
U_full = zeros(4*(2*K+1)*(Nc+1),Nrun-1);
U_perturbation(:,1) = U0;
...
for jj=2:Nrun
    ...
    U_full(:,jj-1) = mat_2_vec(Psi2_phy,Q_phy,W_phy,Phi_phy,Nc,K);
    U_perturbation(:,jj) = mat_2_vec(psi2_phy,q_phy,w_phy,phi_phy,Nc,K);
end
%ii = 1;
